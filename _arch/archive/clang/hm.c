...
void set(hash_map* hm, const char* key, const char* value) {
    ...
    key_value_pair* cur_pair = hm->pairs[index];
    int i = 1;

    // hash collision
    while (cur_pair != NULL) { 
        // overwrite if the key is same
        if (cur_pair != &DELETED_PAIR) && (strcmp(cur_pair->key, key) == 0){ 
            delete_pair(cur_pair);
            hm->pairs[index] = new_pair;
            return;
        }
        // try new index
        index = get_hash(new_pair->key, hm->size, i);
        cur_pair = hm->pairs[index];
        i++;
    } 
    hm->pairs[index] = new_pair;
    hm->count++;
